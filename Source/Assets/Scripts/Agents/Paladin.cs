...
public class Paladin : Agent
{
    ...
    public Paladin(string team, int agentId, int gridId) : base(team, "paladin", agentId, gridId)
    {
    ...
    protected override void loadParameters()
    {
        ...
        mHealth = settings.paladinHealth;
        mDefense = settings.paladinDefense;
        mHealAmount = settings.paladinHealthBuff;
    }
    ...
    private List<Vector2> findHealingLocations()
    {
        ...
        // Check the grid around the paladin, 2 grids in each direction.
        for (int i = -2; i <= 2; i++)
        {
            for (int j = -2; j <= 2; j++)
            {
